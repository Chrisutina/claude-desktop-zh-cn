...
SKIP_ASAR_PATCH="${CLAUDE_SKIP_ASAR_PATCH:-1}"
if [ -z "$ACTION" ]; then
  echo "请选择操作："
  echo
  echo "  [1] 标准汉化    不修改 app.asar，保留 Claude 签名"
  echo "      · 菜单栏、托盘、系统对话框、应用内界面：中文"
  echo "      · Cowork 沙箱/工作区：可能可用"
  echo "      · 不含：在线页面汉化、模型选择器汉化、第三方模型名校验绕过"
  echo "      · 用第三方 API 时需在网关或 CC Switch 中把模型名映射为"
  echo "        Claude / Anthropic 风格名称，否则推理配置无法保存"
  echo
  echo "  [2] 完整汉化    修改 app.asar，汉化最全"
  echo "      · 在 [1] 基础上额外汉化在线页面、主进程菜单、模型选择器，"
  echo "        并绕过第三方模型名校验"
  echo "      · Cowork 沙箱/工作区：必不可用（签名失效）"
  echo
  ...
  case "${action_choice:-1}" in
    1) ACTION="install"; SKIP_ASAR_PATCH="1" ;;
    2) ACTION="install"; SKIP_ASAR_PATCH="0" ;;
    ...
    6)
      ...
      ;;
    *) ACTION="install"; SKIP_ASAR_PATCH="1" ;;
  esac
  echo
fi
...
if [ "$ACTION" = "install" ]; then
  ...
  if [ -n "$SKIP_ASAR_ARG" ]; then
    echo "模式: 标准汉化（不修改 app.asar，保留 Claude 签名）"
  else
    echo "模式: 完整汉化（修改 app.asar；Cowork 沙箱/工作区将不可用）"
  fi
  echo
fi
