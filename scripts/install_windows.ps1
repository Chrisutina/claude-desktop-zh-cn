...
function Merge-LocaleWithBaseline {
    <#
      Mirrors merge_frontend_locale() in patch_claude_zh_cn.py (macOS).
      Claude falls back to the English default message for keys a locale file
      does not define, but not every code path does so gracefully. Writing the
      app's own en-US.json as the base and overlaying our translation guarantees
      the locale file is never missing a key, and drops stale keys that the
      current Claude build no longer ships.
    #>
    param(
        [string]$Source,
        [string]$Baseline,
    ...
    if (-not (Test-Path -LiteralPath $Baseline)) {
        ...
        Write-Host "  installed $Label (direct copy; no en-US baseline present)" -ForegroundColor DarkYellow
        return
    }

    $en = Get-Content -LiteralPath $Baseline -Raw -Encoding UTF8 | ConvertFrom-Json
    $zh = Get-Content -LiteralPath $Source -Raw -Encoding UTF8 | ConvertFrom-Json

    $zhMap = @{}
    foreach ($prop in $zh.PSObject.Properties) {
        $zhMap[$prop.Name] = $prop.Value
    }
    ...
    foreach ($prop in $en.PSObject.Properties) {
        if ($zhMap.ContainsKey($prop.Name)) {
            $merged[$prop.Name] = $zhMap[$prop.Name]
            if ($zhMap[$prop.Name] -ne $prop.Value) {
                $translated++
            }
        } else {
    ...
    foreach ($key in $zhMap.Keys) {
        if (-not $en.PSObject.Properties[$key]) {
            $stale++
        }
    }

    Save-JsonNoBom $Target $merged
    Write-Host "  installed $Label ($translated translated, $fallback English fallback, $stale stale keys dropped)" -ForegroundColor Green
}
